...
export const CountryListDropdown: React.FC<{ 
  countries: Country[], 
...
}> = ({ countries, selectedCountry, onSelectCountry, isOpen, onClose }) => {
  ...
  return (
    <div className={styles.container}>
      {/* Botón de selección */}
      <button onClick={onClose} className={styles.selectButton}>
        ...
        <div className={`${styles.dropdown} ${styles.scrollbar}`}>
          <ul className={styles.list}>
            {countries.map((country, index) => (
              <li 
                key={country.code} 
                onClick={() => { onSelectCountry(country.code); onClose(); }} 
                className={`${styles.listItem} ${highlightIndex === index ? styles.highlighted : ''}`}
              >
...
export const CountryList = CountryListDropdown;
