            ...
            <div className={`${styles['country-input']}`}>
                <div>
                    <div>
                        ...
                        <label htmlFor="" className={`${disabled ? styles['disabled'] : ''} ${size ? styles[size] : ''}`}>{label ? label : ""}</label>
                    </div>
                    <div>
                    ...
                    <Select
                        ...
                        sx={{
                            ...
                            '&.mouse-focus.Mui-focused .MuiOutlinedInput-notchedOutline': {
                                borderColor: error ? 'var(--color-feedback-negative-500)' : 'var(--color-brand-primary-500)',
                                borderWidth: '1px',
                            },
                        }}
                        className={`
                            ${styles['select-country']}
                            ${borderRadius == 'circle' ? styles['select-circle'] : borderRadius == 'semi' ? styles['select-semi'] : null}
                ...
                <div>
                    <div>
                        {info && <Icon data-testid="button-icon" src={info} className={`${disabled ? styles['disabled'] : error ? styles['error'] : ''}`} />}
                        <p className={`${disabled ? styles['disabled'] : error ? styles['error'] : ''} ${size ? styles[size] : ''}`}>{helperText}</p>
                    </div>
                    <div>
                        {info && <Icon data-testid="button-icon" src={info} />}
                    </div>
                </div>
            </div >
        </>
